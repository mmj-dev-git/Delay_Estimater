...
int main() {
    ...
    std::cout << "Render file information:" << std::endl
              ...
              << std::endl
              << "  number of samples: " << captured.num_samples() << std::endl << std::endl;
    ...
    std::vector<float> render_samples_chunk(10000);
    std::vector<float> capture_samples_chunk(10000);
...
// ******** code changes **********************
    auto my_render_iter = render_samples.begin();
    auto my_capture_iter = capture_samples.begin();

    for(int i=0; i<rendered.num_samples()/10000 ;i++){
        std::cout <<"iteration begins"<< std::endl;
        auto my_render_end = std::next(my_render_iter, 10000);
        auto my_capture_end = std::next(my_capture_iter, 10000);
